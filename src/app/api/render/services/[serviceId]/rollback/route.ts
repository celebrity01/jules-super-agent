...
export async function POST(request: NextRequest, { params }: { params: Promise<{ serviceId: string }> }) {
  ...
  try {
    ...
    const res = await fetch(`${BASE}/services/${serviceId}/rollback`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify(body) });
