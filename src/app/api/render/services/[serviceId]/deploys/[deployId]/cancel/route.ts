...
export async function POST(request: NextRequest, { params }: { params: Promise<{ serviceId: string; deployId: string }> }) {
  ...
  try {
    const res = await fetch(`${BASE}/services/${serviceId}/deploys/${deployId}/cancel`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}` } });
