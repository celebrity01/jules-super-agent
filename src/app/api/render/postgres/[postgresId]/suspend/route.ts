...
export async function POST(request: NextRequest, { params }: { params: Promise<{ postgresId: string }> }) {
  ...
  try {
    const res = await fetch(`${BASE}/postgres/${postgresId}/suspend`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}` } });
